...
export const CONTRAST_PAIRS: readonly ContrastPair[] = [
  ...
  /* =====================================================================
     ...
     background must be a single token, so the blend is asserted separately
     via `pillNavBackdrop` (see the contrast test).
     ===================================================================== */
...
/**
 * Alpha of the Pill_Nav's own surface. `SiteHeader` paints
 * `color-mix(in srgb, var(--layer-0) 72%, transparent)` so the
 * `backdrop-filter` blur is visible, which means 28% of whatever band sits
 * beneath the pill shows through. Kept here, next to the pairing table, so the
 * contrast assertion and the component cannot drift apart silently.
 */
